...
export function generateBracket(realPlayers: Participant[]): BracketStructure {
    ...
    const aiNeedded = bracketSize - playerCount;
    const aiPlayers = createAiParticipants(aiNeedded);
        ...
        for (let position = 0; position < matchesInRound; position++) {
            if (round === 0) {
                ...
                matches.push({
                    ...
                    playerAId: allParticipants[playerAIndex]?.oddPlayerId || null,
                    playerBId: allParticipants[playerBIndex]?.oddPlayerId || null
                });
            } else {
                matches.push({
                    roundIndex: round,
